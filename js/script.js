...
(() => {
  ...
  async function handleLoginSubmit(e) {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/auth/login`, {
        ...
        body: JSON.stringify({ loginUser, senha }),
      });
      ...
      showToast(`Bem-vindo(a) de volta, ${data.user.nome}!`, "success");

      // Limpar formulário e navegar para o dashboard
  ...
  async function handleRegisterSubmit(e) {
    ...
    try {
      ...
      showToast(
        "Cadastro criado com sucesso! Bem-vindo(a) ao portal.",
        "success",
      );
  ...
  async function loadAvailableSlots(date) {
    ...
    container.innerHTML = `<div class="spinner-custom" style="width:30px; height:30px; margin:20px auto; grid-column:1/-1;"></div>`;

    try {
      const response = await fetch(
        `${API_BASE}/api/appointments/available-slots?date=${date}`,
      );
      ...
      container.innerHTML = `<div class="text-center text-muted" style="grid-column: 1 / -1;">Não foi possível carregar os horários.</div>`;
      showToast(
        "Erro de comunicação com o servidor ao consultar horários.",
        "danger",
      );
    }
  }
